			...
			if (x > 0 && x < 1024 && scanlineY>0 && scanlineY < 768) {
				if (uv.x >= 0 && uv.x <= 1 && uv.y >= 0 && uv.y <= 1) {
					...
					if (z < this->depthstencilview->texture2d->data[x][scanlineY].r) {//如果该深度值小于z缓冲区的深度值，说明应该显示
						...
						//this->rendertargetview->texture2d->data[x][scanlineY] = PixelShader(uv, Normal, Pos, TBN)+ this->rendertargetview->texture2d->data[x][scanlineY];//add混合，检查重复绘制，漏绘
					}
				}
			}
		}
...
void DeviceContext::fillTopFlatTriangle(SimpleVertex v1, SimpleVertex v2, SimpleVertex v3)
{
	...
	//考虑透视矫正算世界坐标增量
	VECTOR4 Posdxnnumerator = v1.PosWorld * v1.Pos.w * dx.x + v2.PosWorld * v2.Pos.w * dx.y + v3.PosWorld * v3.Pos.w * dx.z;//在x增量
	VECTOR4 Posdynnumerator = v1.PosWorld * v1.Pos.w * dy.x + v2.PosWorld * v2.Pos.w * dy.y + v3.PosWorld * v3.Pos.w * dy.z;//在y增量
			...
			if (x > 0 && x < 1024 && scanlineY>0 && scanlineY < 768) {
				...
				if (uv.x >= 0 && uv.x <= 1 && uv.y >= 0 && uv.y <= 1) {
					...
					if (z < this->depthstencilview->texture2d->data[x][scanlineY].r) {//如果该深度值小于z缓冲区的深度值，说明应该显示
						...
						this->depthstencilview->texture2d->data[x][scanlineY].r = z;//把z存入z缓冲区
						this->rendertargetview->texture2d->data[x][scanlineY] = PixelShader(uv, Normal, Pos, TBN);//正常绘制模式
						//this->rendertargetview->texture2d->data[x][scanlineY] = PixelShader(uv, Normal, Pos, TBN) + this->rendertargetview->texture2d->data[x][scanlineY];//add混合，检查重绘漏绘
					}
				}
			}
		}
...
{
	SimpleVertex temp;//保证v1在最上面，v2在中间
...
void StringSplit(std::string s, char splitchar, std::vector<std::string>& vec)
{
	...
	for (int i = 0; i < length; i++) {
		if (s[i] == splitchar && i == 0) {//第一个就遇到分隔符
			start += 1;
		}
...
void ReadObjFile(std::string path, Mesh* obj)
{
	...
	if (in) {//获取文件成功
		...
		while (getline(in, txt))//line中不包括每行的换行符
		{
			...
			//索引编号
			//在面的数据中多了贴图左边uv和法线的索引号，索引号分别用左斜线(/)隔开
